...
import { ArrowLeft, ChevronRight, PartyPopper, Ticket } from 'lucide-react';
...
const STORE_ITEMS = [
    ...
    { id: 'item_1500', title: '식사 교환권', price: 2000, desc: '김밥, 햄버거 세트 등', icon: '🍱', color: '#e76f51' },
    { id: 'item_3000', title: '애슐리퀸즈', price: 3000, desc: '프리미엄 뷔페 이용권', icon: '🍽', color: '#8b5cf6' },
];
...
export default function Store() {
    ...
    const currentPoints = useMemo(() => (selectedUserId ? getUserCurrentPoints(selectedUserId) : 0), [selectedUserId, getUserCurrentPoints, myCoupons, totalScore, spentPoints]);
    ...
    const handleBuy = (item: (typeof STORE_ITEMS)[0]) => {
        ...
        if (window.confirm(`[${item.title}]을(를) ${item.price}P로 교환할까요?`)) {
            const success = buyCoupon(selectedUserId, { couponId: item.id, title: item.title, price: item.price });
            if (success) {
                confetti({ particleCount: 100, spread: 70, origin: { y: 0.6 }, colors: ['#0064FF', '#60a5fa', '#fff'] });
                alert('교환 완료!');
    ...
    if (!selectedUserId) {
        return (
            <div className="flex flex-col min-h-screen bg-[#F2F6FF] p-6 items-center justify-center">
                ...
                <button onClick={() => navigate('/')} className="px-6 py-3 bg-[#0064FF] text-white rounded-[14px] font-black shadow-[0_4px_12px_rgba(0,100,255,0.3)]">
                    홈으로
                </button>
            </div>
        );
        ...
        <div className="flex flex-col min-h-screen bg-[#F2F6FF]">
            <div className="px-5 pt-14 pb-4">
                ...
                <div className="flex bg-white border border-blue-100 rounded-[16px] p-1 shadow-sm">
                    <button onClick={() => setActiveTab('shop')} className={`flex-1 py-2.5 text-[13px] font-black rounded-[12px] transition-all ${activeTab === 'shop' ? 'bg-[#0064FF] text-white shadow-sm' : 'text-[#8B95A1]'}`}>
                        교환 목록
                    </button>
                    <button onClick={() => setActiveTab('inventory')} className={`flex-1 py-2.5 text-[13px] font-black rounded-[12px] transition-all ${activeTab === 'inventory' ? 'bg-[#0064FF] text-white shadow-sm' : 'text-[#8B95A1]'}`}>
                        내 쿠폰 {myCoupons.length > 0 && `(${myCoupons.length})`}
                    </button>
                </div>
            </div>
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex items-center gap-4 mb-4">
                                            <div className="w-12 h-12 rounded-[14px] flex items-center justify-center text-2xl" style={{ backgroundColor: `${item.color}20`, border: `1px solid ${item.color}40` }}>
                                                {item.icon}
                                            </div>
                                            <div>
                                                <h3 className="font-black text-[15px] text-[#191F28]">{item.title}</h3>
                                                <p className="text-[12px] text-[#8B95A1] font-bold">{item.desc}</p>
                                            </div>
                                        </div>
                                        <button
                                        ...
                                        >
                                            {canAfford ? (
                                                ...
                                                `${item.price.toLocaleString()}P 필요`
                                            )}
                                        </button>
                                    </motion.div>
                                );
                            ...
                            {myCoupons.length === 0 ? (
                                <div className="text-center py-20">
                                    <div className="w-14 h-14 bg-blue-50 rounded-full flex items-center justify-center mx-auto mb-4">
                                        <Ticket className="w-7 h-7 text-blue-200" />
                                    </div>
                                ...
                                myCoupons.map((coupon, idx) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="w-11 h-11 rounded-[12px] flex items-center justify-center text-xl" style={{ backgroundColor: `${storeItem?.color || '#0064FF'}20` }}>
                                                {storeItem?.icon || '🎁'}
                                            </div>
